---
name: ai-cost-per-tenant-attribution
description: Use when building the operational pipeline that attributes every AI request to a tenant in real time — token accounting at the gateway, model price tables, per-tenant cost dashboards, anomaly detection, plan-cost guardrails, soft/hard spend ceilings, kill-switches, and finance reconciliation. Distinct from `ai-cost-modeling` (design-time unit-cost reasoning) and `ai-usage-metering-and-billing` (the commercial billing recipe).
metadata:
  portable: true
  compatible_with:
  - claude-code
