---
name: ai-agent-observability-and-replay
description: Use when designing the observability stack for agentic features — trace per task, per-step span, tool I/O capture, deterministic replay tooling, "what would the agent do differently" debugging, and task-level dashboards. Distinct from `ai-observability-and-debugging` (request-level AI observability) by focusing on multi-step task trajectories.
metadata:
  portable: true
  compatible_with:
  - claude-code
