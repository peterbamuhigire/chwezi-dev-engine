...
description: Use when designing PLG motions for a SaaS product — freemium tiers, PQL definition, activation flows with time-to-value targets, in-app upgrade prompts, viral loops, NPS surveys, feature flags for gradual rollout, and PostHog-based product analytics for funnel and cohort tracking.
metadata:
  portable: true
  compatible_with:
  - Codex
...
Do not approve a PLG intervention without an observable activation event, segment denominator, guardrail, experiment owner, and rollback or stop rule.

## Outputs

| Artefact | Consumer | Acceptance condition |
|---|---|---|
| PLG experiment and instrumentation plan | Product, growth, and revenue teams | Activation event, PQL threshold, cohort cuts, upgrade trigger, guardrail, owner, and stop rule are testable from named telemetry |
