...
7. Use fictional, synthetic, culturally appropriate identities. Human-facing names, titles, descriptions, complaints, notes, and narratives must sound natural and domain-realistic; do not label them `DEMO`, `TEST`, `SAMPLE`, use numbered-placeholder prose, or emit gibberish. Values that could be mistaken for authentic legal, regulatory, identity, tax, or financial identifiers - including national IDs, passports, TINs, bank accounts, and equivalent account numbers - must contain the literal marker `DEMO`. If a production-format validator cannot accept that marker, use an explicitly supported sandbox identifier type or return `BLOCKED_CAPABILITY`; never fall back to a realistic unmarked identifier. Technical fixture keys and manifest-only ownership markers may contain `DEMO`, but must not render as business content. Never store reusable production secrets or real regulated data.
...
6. **Design identities and separation of duties.** Create fictional stable fixture keys, natural human-facing identities and content, roles, departments, supervisors, facility grants, account states, preparer/approver pairs, suspension/offboarding cases, and preserved historical attribution. Keep seed markers out of displayed names and prose; visibly mark sensitive legal/financial identifiers with `DEMO` as required by rule 7. For reusable human-name variety, load `references/synthetic-name-bank-standard.md` and its four CSV assets. Select given names by gender and naming tradition, select surnames from the matching tradition pool, derive the result from a stable fixture key, record the asset checksum, and reject blank, duplicate, under-sized, or unverified pools.
...
- Do not expose `DEMO`, `TEST`, `SAMPLE`, fixture codes, lorem ipsum, numbered placeholders, or gibberish in human-facing names, titles, descriptions, complaints, notes, or narratives.
- Do not generate a realistic-looking unmarked national ID, passport, TIN, bank account, or equivalent sensitive identifier; use a `DEMO`-marked or explicitly sandbox-typed value, or block the scenario.
