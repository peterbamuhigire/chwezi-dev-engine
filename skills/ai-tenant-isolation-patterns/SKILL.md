---
name: ai-tenant-isolation-patterns
description: Use when designing tenant isolation for AI assets — vector stores, prompts, fine-tunes, eval datasets, conversation logs, citation grounding — in a multi-tenant SaaS. Covers silo vs pool-with-filter decisions, cryptographic partitioning, namespace strategies, leakage prevention, and data-bleed test suites. Distinct from `multi-tenant-saas-architecture` (which handles transactional-data isolation only).
metadata:
  portable: true
  compatible_with:
  - claude-code
