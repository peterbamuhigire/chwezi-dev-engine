---
name: ai-rag-multi-tenant
description: Use when building or hardening RAG in a multi-tenant SaaS — per-tenant ingestion pipelines, chunking strategies per tier, embedding model selection per tier, vector store partitioning, retrieval security, citation UX, and the per-tenant KB service contract. Complements `ai-rag-patterns` (general RAG) with full multi-tenancy treatment.
metadata:
  portable: true
  compatible_with:
  - claude-code
