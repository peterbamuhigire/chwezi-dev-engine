...
metadata:
  ...
  - claude-code
  - codex
  origin: "Adapted from affaan-m/ECC skills/inherit-legacy-style/SKILL.md"
---

> Inactive alias. Route to `skills/sdlc-meta/ai-assisted-development` through `docs/skill-aliases.yml`; this file is retained for historical content.

# Inherit Legacy Style

Prevents AI code style drift on legacy projects by scanning the codebase for implicit conventions
across four meta-architecture dimensions, resolving conflicts with the user one at a time, and
crystallizing the consensus into an enforceable `.ai-style-rules.md`. Fully language- and
...
- User types `/inherit-legacy-style` or asks to onboard AI coding help onto an older codebase
...
- `sdlc-meta/codebase-onboarding`-equivalent context (this engine's onboarding conventions, where present)
- `code-review` (this session) for reviewing diffs against the crystallized rules once generated
- `sdlc-meta/ai-assisted-development` for the broader AI-assisted workflow this feeds into
