...
description: Use when designing memory for agentic features in a multi-tenant SaaS — short-term (turn buffer), working (episode), long-term (semantic memory per tenant), forget-on-erase cascade, cross-conversation isolation, and the contract with GDPR erasure. Distinct from `ai-rag-multi-tenant` (knowledge-base ingestion) by being about the *agent's own derived memory* — what it learned about a tenant from prior tasks.
metadata:
  portable: true
  compatible_with:
  - claude-code
