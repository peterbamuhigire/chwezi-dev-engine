---
name: ai-agent-eval
description: Use when evaluating agentic features — task success rate, step efficiency, tool-choice quality, hallucinated tool args, irreversible-action rate, intervention rate, time-to-complete. Includes golden-task construction and replay-based eval. Distinct from `ai-eval-harness` (eval of single-shot prompts) by being task-level / trajectory-level.
metadata:
  portable: true
  compatible_with:
  - claude-code
