---
name: ai-agent-tool-catalogue-and-action-gating
description: Use when building the tool registry that an agent in a multi-tenant SaaS can call — typed schemas, per-tenant allow-lists, reversible-vs-irreversible classification, side-effect budgets, tool version pinning, deprecation, and the contract between tool authors and agent authors. Distinct from `ai-agents-tools` (the loop and ReAct pattern) by being specifically about the *catalogue* as a control-plane artifact.
metadata:
  portable: true
  compatible_with:
  - claude-code
