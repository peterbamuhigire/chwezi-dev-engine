...
```bash
# In a GitHub Actions job with `permissions: id-token: write`, cosign keyless
# signing picks up the workflow's ambient OIDC token. Do not pass a GitHub PAT
# (`gh auth token`) as an identity token; it is not an OIDC token.
cosign sign --yes registry.example.com/web@sha256:abc...
