# Iterative Retrieval for Context-Starved Subagents

Absorbed from the retired `iterative-retrieval` skill (origin: adapted from affaan-m/ECC
`skills/iterative-retrieval/SKILL.md`). Load this reference when the "corpus" is a codebase or
workspace being explored for one subagent's task, not a document/knowledge-base RAG system:
a subagent is spawned with limited context and does not yet know which files are relevant, what
patterns exist, or what terminology the project uses.

Boundaries: general document RAG stays in the parent `ai-rag-patterns` skill; the handoff
contract between agents stays in `skills/ai/ai-agent-multi-agent-coordination`; verifying the
subagent's *output* belongs to `sdlc-meta/santa-method`.
...
- Refined context set (paths plus relevance score) and a record of each pass's decisions and gaps.
...
- Every pass is bounded (max 3 cycles) and traceable to the stated goal and boundary.
- The gap list is explicit after each pass; refinement targets gaps, not a broader rerun.
- Never guess missing context; dispatch a bounded retrieval pass instead.

Evidence/currentness: pattern guidance with no version-, API- or vendor-specific claims; no
external verification required (reviewed 2026-09-24).
