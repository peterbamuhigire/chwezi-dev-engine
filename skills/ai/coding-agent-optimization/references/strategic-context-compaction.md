...
Load when a long coding-agent session is approaching a context limit, or sits between task
phases (research, plan, implement, test), and you must decide whether to compact manually now
rather than let auto-compaction fire mid-task and discard state you still need.

Absorbed from the retired `sdlc-meta/strategic-compact` skill (origin: adapted from affaan-m/ECC
`skills/strategic-compact/SKILL.md`, doctrine only). Runner-specific compaction commands and
limits belong in the [Claude Code](claude-code.md) or [Codex](codex.md) adapter; verify them
against the installed runner before citing them.
