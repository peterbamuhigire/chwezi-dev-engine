...
description: Use when designing or building an LLM gateway for provider abstraction, tenant-aware routing, fallbacks, quotas, residency, audit logging, cost capture, and kill-switch enforcement.
metadata:
  portable: true
  compatible_with:
  - claude-code
...
|---|---|---|
| Domain evidence | yes | provider inventory, tenant plans, residency constraints, request schema, budgets, fallback policy, and kill-switch owner |

## Outputs

- Produce: gateway API contract, routing table, quota enforcement, audit/cost events, fallback chain, and operational runbook.
