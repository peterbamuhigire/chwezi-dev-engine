# Structured-Text Extraction: Regex vs LLM

Load when parsing structured or semi-structured text (forms, invoices, receipts, quiz or exam
content, scraped listings, document sections) and deciding whether to use deterministic regex,
an LLM call, or a hybrid pipeline that sends only low-confidence items to the model.

Absorbed from the retired `sdlc-meta/regex-vs-llm-structured-text` skill (origin: adapted from
affaan-m/ECC `skills/regex-vs-llm-structured-text/SKILL.md`). When the LLM path is used, apply
the structured-output, retry, and usage-metering patterns in `SKILL.md`, and send the call
through `ai-model-gateway` in multi-tenant production.
