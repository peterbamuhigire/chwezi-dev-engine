---
name: ai-usage-metering-and-billing
description: Use when turning AI usage (tokens, calls, generations, agent steps) into billable units, prepaid credits, overage, and Stripe (or comparable) metered billing. Covers billable-dimension design, unit normalisation, included allowances, overage tiers, prepaid credit ledgers, Stripe Meters / usage records integration, invoice copy, and proration. Distinct from `ai-cost-per-tenant-attribution` (internal cost) and `ai-entitlements-and-feature-gating` (gate vs meter).
metadata:
  portable: true
  compatible_with:
  - claude-code
